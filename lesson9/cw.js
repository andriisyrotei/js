...
simpsons.forEach(value => {
    let div = document.createElement('div');
    div.classList.add('member');
    for (const divKey in value) {
        if (divKey !== 'photo') {
            let divElement = document.createElement('div');
            divElement.innerText = `${divKey} - ${value[divKey]}`
            div.appendChild(divElement);
        } else {
            let imgElement = document.createElement('img');
            imgElement.src = value[divKey];
...
coursesArray.forEach(value => {
    ...
    titleH2.innerText = value.title;
    monthDiv.innerText = value.monthDuration;
    hourDiv.innerText = value.hourDuration;
    ...
    value.modules.forEach(item => {
        let moduleDiv = document.createElement('div');
        moduleDiv.innerText = item;
